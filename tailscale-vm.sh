...
if whiptail --backtitle "Proxmox VE Helper Scripts" --title "Tailscale VM" --yesno "This will create a New Debian VM with Tailscale. Proceed?" 10 58; then
  :
else
...
function default_settings() {
  ...
  echo -e "${DEFAULT}${BOLD}${DGN}Authkey: ${BGN}Default${CL}"
  echo -e "${GATEWAY}${BOLD}${DGN}Start VM when completed: ${BGN}yes${CL}"
  echo -e "${CREATING}${BOLD}${DGN}Creating a Tailscale VM using the above default settings${CL}"
}
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Hostname" 8 58 tailscale --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="tailscale"
      echo -e "${HOSTNAME}${BOLD}${DGN}Hostname: ${BGN}$HN${CL}"
    else
  ...
  if AUTHKEY1=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Tailscale Authkey (leave blank for default)" 8 58 --title "Tailscale Authkey" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $AUTHKEY1 ]; then
      AUTHKEY1="Default"
      AUTHKEY=""
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create a Talescale VM?" --no-button Do-Over 10 58); then
    echo -e "${CREATING}${BOLD}${DGN}Creating a Tailscale VM using the above advanced settings${CL}"
  else
    header_info
...
msg_ok "Added Tailscale to Debian 13 Qcow2 Disk Image successfully"
...
msg_info "Creating a Tailscale VM"
...
<div align='center'>
  ...
  <h2 style='font-size: 24px; margin: 20px 0;'>Tailscale VM</h2>
...
msg_ok "Created a Tailscale VM ${CL}${BL}(${HN})"
if [ "$START_VM" == "yes" ]; then
  msg_info "Starting Tailscale VM"
  qm start $VMID
